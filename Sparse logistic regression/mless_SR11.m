function [iter, x_opt, F_opt] = mless_SR11(x0, f, gradfun, g, opts)
% 设置参数
params = struct('nu1', 1e-8, ...% 公式(33)所需参数
    'thetak', 0.1, ... % 公式(31)所需参数
    'delta', 1e-3, 'beta', 0.5, ...% 线搜索参数
    'max_iter', 10000, 'tol', 1e-6,'ftol',1e-9,'lambda', 1e-3);
...
while k<params.max_iter
    ...
    % 计算gammak(66)
    gammak = sTs/sTz - sqrt((sTs/sTz)^2 - sTs/zTz);
    
    % SR1公式对应Phi值
    Phik_x = -(sTz^2) / (sTs*zTz - (sTz^2));
    Phi = gammak*sTz / ((gammak*z-s)'*s);
    if Phi < (0.95*Phik_x)
        Phik = -0.95*Phik_x;
    else
        Phik = Phi;
    end
    PhikH = Phik_x*(1-Phik) / (Phik_x-Phik);
